...
def test(ctx, exercise =_get_exercise()):
    if not exercise:
        print("ERROR: either run `inv test` within an exercise directory or pass an exercise to the invoke command. (ex. `inv test --exercise 1`)")
        return
    ...
    ctx.run("go install")
    print("Running Test...")
    ctx.run(f"go test -v exercise_{exercise}_test.go")
